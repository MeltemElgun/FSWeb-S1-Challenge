...
console.log("indekseGoreFenomen");
function indekseGoreFenomen(dizi,İndeks) {
  let fenomen=dizi[İndeks].profile;
  return (İndeks+". indekste bulunan fenomen:"+" "+fenomen);
}
console.log(indekseGoreFenomen(fenomenler,0));
...
function fenomenSil(dizi,sayi) {
  ...
  let yeni= yeniDizi.splice(sayi,1);
let son=[];
...
*/
console.log("enFenomenler");
function enFenomenler(dizi) {

  let dönen=[];
for (let i = 0; i < dizi.length; i++) {
   let sonuc= dizi[i].followers;
   if (sonuc>100000000) {

      dönen.push(dizi[i].profile);
  }
}
return dönen;
}
...
*/
console.log("fenomenGonderimSayisi");
function fenomenGonderimSayisi(dizi,adi){
  for (let i = 0; i < dizi.length; i++) {
    let indeks = dizi[i].profile;
    if (indeks==adi) {
      return dizi[i].posts;
    }
  }
...
function platformaGoreCokGonderiYapanFenomen(dizi,adi){
